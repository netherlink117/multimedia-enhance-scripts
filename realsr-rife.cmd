...
:cals
set /a "osize=%osize%*4"
set /a "owidth=%owidth%*4"
set /a "oheight=%oheight%*4"
...
:suma
@REM cls
echo NOTE: This process will use realsr-ncnn-vulkan %scount% times to increaze size and rife-ncnn-vulkan %fcount% times to increase frames.
...
:init
cls
set "scalator=%scount%realsr_models_DF2K_JPEG"
...
:besc
set /a "scountaux=%scount%+1"
if %scount% gtr 0 (
  if exist "%sworkspace%\scaled%scountaux%\000000.png" (
    mkdir "%sworkspace%\scaled%scount%" 2>> "%sworkspace%\log.txt"
    realsr-ncnn-vulkan.exe -v -s 4 -m "models-DF2K_JPEG" -i "%sworkspace%\scaled%scountaux%" -o "%sworkspace%\scaled%scount%"
  ) else (
    mkdir "%sworkspace%\scaled%scount%" 2>> "%sworkspace%\log.txt"
    realsr-ncnn-vulkan.exe -v -s 4 -m "models-DF2K_JPEG" -i "%sworkspace%\input" -o "%sworkspace%\scaled%scount%"
  )
  set /a "scount=%scount%-1"
  goto :besc
) else (
  if not exist "%sworkspace%\scaled%scountaux%\000000.png" (
    mkdir "%sworkspace%\scaled%scount%" 2>> "%sworkspace%\log.txt"
    copy /y "%sworkspace%\input\*" "%sworkspace%\scaled%scount%"
  ) else (
    mkdir "%sworkspace%\scaled%scount%" 2>> "%sworkspace%\log.txt"
    copy /y "%sworkspace%\scaled%scountaux%\*" "%sworkspace%\scaled%scount%"
  )
)
...
if %fcount% gtr 0 (
  if exist "%sworkspace%\interpolated%fcountaux%\000001.png" (
    ...
    mkdir "%sworkspace%\interpolated%fcount%" 2>> "%sworkspace%\log.txt"
    rife-ncnn-vulkan.exe -v -u -j "1:1:1" -m "rife-v3.1" -f "%%06d.png" -i "%sworkspace%\scaled0" -o "%sworkspace%\interpolated%fcount%"
  )
  set /a "fcount=%fcount%-1"
  goto :bint
) else (
  if not exist "%sworkspace%\interpolated%fcountaux%\000001.png" (
    mkdir "%sworkspace%\interpolated%fcount%" 2>> "%sworkspace%\log.txt"
    copy /y "%sworkspace%\scaled0\*" "%sworkspace%\interpolated%fcount%"
  ) else (
    mkdir "%sworkspace%\interpolated%fcount%" 2>> "%sworkspace%\log.txt"
